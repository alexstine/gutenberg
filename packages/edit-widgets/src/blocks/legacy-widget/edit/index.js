...
import { STORE_NAME as editWidgetsStoreName } from '../../../store';
...
export default withSelect( ( select, { clientId, attributes } ) => {
	...
	let widgetId = select( editWidgetsStoreName ).getWidgetIdForClientId(
		clientId
	);
	const widget = select( editWidgetsStoreName ).getWidget( widgetId );
	const widgetArea = select( editWidgetsStoreName ).getWidgetAreaForClientId(
		clientId
	);
